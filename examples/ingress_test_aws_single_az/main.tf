...
resource "juju_model" "mysql_model" {

  name = "mysql-k8s"
  cloud {
    name = "performance-k8s"
  }

  credential = "performance-k8s"
...
resource "juju_application" "mysql" {
  ...
  name = "mysql"
  model = juju_model.mysql_model.name
  charm {
    name = "mysql-k8s"
    channel = "8.0/edge"
    series = "jammy"
...
resource "juju_integration" "mysql_and_router" {
...
resource "juju_offer" "database" {
  ...
  model            = juju_model.mysql_model.name
  application_name = juju_application.mysql_router.name
  endpoint         = "database"
}


// Integrate with sysbench
resource "juju_integration" "sysbench-1" {
